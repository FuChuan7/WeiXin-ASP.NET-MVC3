...
namespace Tencent.Weixin
{
    public class JSSDK
    {
        ...
        public SignPackage GetSignPackage(JsApiEnum jsapi, string GrantType, string AppID, string AppSecret)
        {
            ...
            return GetSignPackage(url, jsapi,GrantType,AppID,AppSecret);
        }
        /// <summary>
        /// 获取jssdk签名配置对象
        /// </summary>
        ...
        /// <param name="jsapi">JsApiEnum,如:JsApiEnum.scanQRCode|JsApiEnum.onMenuShareQQ</param>
        /// <returns>微信公众平台JsSdk的配置对象</returns>
        public SignPackage GetSignPackage(string url, JsApiEnum jsapi, string GrantType, string AppID, string AppSecret)
        {

            BaseMsg baseMsg = WxController.getInstance().getAccessToken(GrantType, AppID, AppSecret);
            
                    AccessToken accessToken = (AccessToken)baseMsg;
            string noncestr = this.CreateNonceStr(16);
            string jsapi_tkcket = this.GetJsApiTicket(accessToken);
