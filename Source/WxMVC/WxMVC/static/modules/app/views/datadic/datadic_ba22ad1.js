...
var model = avalon.define({
    ...
    load: function () {
        ajax.datadic.List(model.limit, model.currentPage, model.pid).done(function (data) {
    ...
    Save: function () {
        if ($.trim(model.CurrentObj.Value) == '') {
            rootVm.$alert('提示', '参数值不能为空', 'error');
            $("#AddOrUpdate_modal").modal('hide');
            return;
        }
        ajax.datadic.save(model.CurrentObj.ID, model.CurrentObj.Code, model.CurrentObj.Name, model.CurrentObj.Des, model.CurrentObj.Value, 0).done(function (data) {
            if (data.success) {
