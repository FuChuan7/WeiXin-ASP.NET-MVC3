...
var model = avalon.define({
    ...
    isAdd: true,
    load: function () {
        ajax.pageCategory.List(model.limit, model.currentPage).done(function (data) {
    ...
    ShowAddCategory: function () {
        ...
        model.ID = 0;
        model.isAdd = true;
        $("#Add_PageCategory_modal").modal('show');
    },
    ...
    ShowDelete: function (obj) {
        model.Name = obj.Name;
        model.ID = obj.ID;
        $("#Delete_PageCategory_modal").modal('show');
    },
    Delete: function () {
        ajax.pageCategory.del(model.ID).done(function (data) {
            if (data.success) {
                rootVm.$alert('提示', data.msg, 'success');
                $("#Delete_PageCategory_modal").modal('hide');
                model.load();
            } else {
                rootVm.$alert('提示', data.msg, 'success');
            }
    ...
    SaveCategory: function () {
        if ($.trim(model.Name) == '') {
            ...
            $("#Add_PageCategory_modal").modal('hide');
            return;
        }
        ajax.pageCategory.save(model.ID, model.Name).done(function (data) {
            if (data.success) {
                rootVm.$alert('提示', data.msg, 'success');
                $("#Add_PageCategory_modal").modal('hide');
                model.load();
